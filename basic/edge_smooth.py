...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='dragonball', help='dataset_name')
...
def make_edge_smooth(dataset_name, img_size) :
    now_path = os.getcwd()
    check_folder(now_path +'/dataset/{}/{}'.format(dataset_name, 'smooth'))

    file_list = glob(now_path +'/dataset/{}/{}/*.*'.format(dataset_name, 'style'))
    save_dir = now_path +'/dataset/{}/smooth'.format(dataset_name)

    kernel_size = 5
    kernel = np.ones((kernel_size, kernel_size), np.uint8)
    gauss = cv2.getGaussianKernel(kernel_size, 3)
